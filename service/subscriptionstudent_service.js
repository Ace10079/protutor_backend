...
class StudentPlanServices{
    static async registerStudentPlan(email,student_id,fname,plan_name,plancost,status,tnx_id,date,count,address,phone){
        try{
            var sub_id = await IdcodeServices.generateCode("StudentPlan");
            const createUser = new StudentPlanModel({sub_id,email,student_id,fname,plan_name,plancost,status,tnx_id,date,count,address,phone});
    ...
    static async updateStudentPlan(sub_id,student_id,fname,plan_name,plancost,status,tnx_id,date,count,address,phone){
        try {
            var query = {sub_id :sub_id};
            var values = {$set : {student_id : student_id,fname : fname,plan_name : plan_name,plancost : plancost,status : status,tnx_id : tnx_id,date : date,count:count,address:address,phone:phone}};
